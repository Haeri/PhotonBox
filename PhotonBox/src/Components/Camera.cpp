...
bool Camera::frustumTest(ObjectRenderer* object)
{
	...
	{
		if(object->getAABB().getMinBoundGlobal().dot(_frustum[i].normal) + _frustum[i].distance <= 0 && 
			object->getAABB().getMaxBoundGlobal().dot(_frustum[i].normal) + _frustum[i].distance <= 0){
		...
		//{
			//std::cout << "Culling " << object->entity->name << std::endl;
			return false;
		}
	}
	return true;
