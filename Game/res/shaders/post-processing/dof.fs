...
void main() 
{
	...
	c_depth = -texture(gPosition, TexCoords).z;
	c_color = texture(renderTexture, TexCoords).rgba;
	...
	/*
	if(-texture(gPosition, TexCoords).z == 0)
		FragColor = vec4(1, 0, 0, 1);
	else
		FragColor = vec4(0, 0, 0, 1);
	*/
/*
	if(textureSize(renderTexture, 0) == textureSize(gPosition, 0))
	{
...
vec4 c_3x3()
{
    ...
    for (int y = -1; y <= 1; ++y) 
    {
		for (int x = -1; x <= 1; ++x) 
        {
            ...
            float i_depth = texture(gPosition, TexCoords + (offset * boost + rand_vecs[(y+1) * 3 + (x+1)] * 0.001)).z;

            if(i_depth == 0 || -i_depth > shallow)
            {
            	ret += texture(renderTexture, TexCoords + (offset * boost + rand_vecs[(y+1) * 3 + (x+1)] * 0.00c1)).rgba * weight_3x3[(y+1) * 3 + (x+1)];
            }
            else
            {
...
vec4 c_5x5()
{
	...
	for (int y = -2; y <= 2; ++y) 
    {
		for (int x = -2; x <= 2; ++x) 
        {
            ...
            float i_depth = texture(gPosition, TexCoords + (offset * boost)).z;
